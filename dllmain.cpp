...
void DrawMenu() {
	...
	if (ZeroGUI::Window("Senex Cheats Internal .gg/senexcheats", &pos, Vector2{ 500.0f, 400.0f }, ShowMenu))
	{
		//Simple Tabs
		static int tab = 0;
		...
		if (tab == 1) {
			...
			ZeroGUI::Checkbox("Skeleton Esp( buggy )", &Settings::SkeletonESP);
		...
		if (tab == 2) {
			...
			ZeroGUI::Checkbox("Projectile Bullet TP", &Settings::SniperTp);
			ZeroGUI::Checkbox("Fov Changer", &Settings::FovChanger);
			ZeroGUI::SliderFloat("Fov Value", &Settings::FovChanger_Value, 0.0f, 180.0f);
		}
		if (tab == 3) {
			ZeroGUI::SliderFloat("Chams Rainbow Speed", &rainbowspeed, 0.0001f, 0.005f);
		}
	}
...
VOID Main()
{
	...
	ue::cached::signatures::GetNameByIndex = sdkUssage->PatternScan(ue::cached::Base, xorstr("40 53 48 83 EC 20 33 C0 48 8B DA 48 89 02 48 89 42 08 E8 ? ? ? ? 48 8B C3 48 83 C4 20 5B C3"), 0, false); if (!ue::cached::signatures::GetNameByIndex) safe_call(MessageBox)(0, xorstr(L"Failed to find GetNameByIndex"), 0, 0); //48 89 ? ? ? 48 89 ? ? ? 48 89 ? ? ? 41 ? 48 81 EC ? ? ? ? 48 8B ? ? ? ? ? 48 33 ? 48 89 ? ? ? ? ? ? 8B 01
